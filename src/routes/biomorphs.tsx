...
function BiomorphsSimulation() {
	...
	const [geneEditValues, setGeneEditValues] = useState<number[]>([
		0, 0, 0, 0, 0, 0, 0, 0, 0,
	]);
	...
	// Create a ref for auto evolution toggle to avoid dependency issues
	...
	}, [biomorphs, editingGenes, zoomedBiomorph, autoEvolving, selectedBiomorph]);

	// Calculate biomorph fitness based on different criteria
	const calculateFitness = (
		genes: number[],
		fitnessType: string = "balanced",
	): number => {
			...
			<div className="max-w-6xl mx-auto">
					...
					<div className="flex flex-wrap items-center justify-between gap-4 mb-4">
						<div>
							<div className="text-lg font-semibold">
								...
								<span className="mr-4">
									History: {history.length} biomorphs
								</span>
								{selectedBiomorph?.fitness !== undefined && (
									<span>
										Current Fitness: {selectedBiomorph.fitness.toFixed(2)}
									</span>
								)}
							</div>
									...
									<div key={`gene-${index}`} className="flex flex-col">
										...
										<div className="flex items-center">
											<input
												...
												value={value}
												onChange={(e) =>
													handleGeneValueChange(index, Number(e.target.value))
												}
											...
											<input
												...
												onChange={(e) =>
													handleGeneValueChange(
														index,
														Math.max(-10, Math.min(10, Number(e.target.value))),
													)
												}
							...
							<div className="mt-4 flex justify-end">
								<button
									onClick={() => {
										...
										const previewBiomorph =
											createCustomBiomorph(geneEditValues);
								...
								<div>
									...
									<div className="text-sm text-gray-400">
										{comparisonBiomorphs.length === 0 &&
											"No biomorphs selected"}
										{comparisonBiomorphs.length === 1 &&
											"One biomorph selected - select another to compare"}
										{comparisonBiomorphs.length === 2 && (
											<span>
												Comparing two biomorphs - Genetic distance:{" "}
												{geneticDistance(
													comparisonBiomorphs[0],
													comparisonBiomorphs[1],
												)}
											</span>
										)}
									</div>
				...
				{showLineage && lineage.length > 1 && (
					<div className="bg-gray-800 rounded-xl p-4 mb-8">
						<div className="flex items-center justify-between mb-4">
							...
							<div className="text-sm text-gray-400">
								Generation {lineage[0]?.generation} to{" "}
								{lineage[lineage.length - 1]?.generation}
							</div>
						</div>
						...
						<div className="relative">
							...
							<div className="flex items-center">
								...
								<button
									...
									onClick={() => {
										const container =
											document.getElementById("lineage-carousel");
										if (container) {
							...
							<div className="mt-4 text-center text-sm text-gray-400">
								This lineage shows the evolutionary path from the first ancestor
								to the current biomorph.
								<br />
								Click on any biomorph to select it and continue evolution from
								there.
							</div>
						</div>
					</div>
									...
									return (
										...
										>
											...
											<div className="flex justify-between">
												...
												<div
													className={`${difference > 0 ? "text-amber-400" : "text-gray-500"}`}
												>
				...
				<div className="bg-gray-800 rounded-xl p-4">
					...
					<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
						...
						{biomorphs
							.slice(0, autoEvolving ? 12 : biomorphs.length)
							.map((biomorph) => {
								...
								const isFittest =
									...
									(autoEvolving
										...
										: biomorphs.every(
												(b) => (b.fitness || 0) <= (biomorph.fitness || 0),
											));
								...
								const distanceFromSelected =
									...
									comparisonBiomorphs.length === 1 &&
									comparisonBiomorphs[0]
										? geneticDistance(biomorph, comparisonBiomorphs[0])
										: null;

								// Optimized class calculation - avoid string template for better performance
								let classes =
									"w-full bg-gray-900 rounded-lg p-2 cursor-pointer transition-all hover:scale-105";
								...
								return (
									...
									>
										<div className="relative">
											...
											{biomorph.fitness !== undefined &&
												(autoEvolving || comparisonMode) && (
													<div
														className={`absolute top-1 right-1 rounded-full px-1.5 py-0.5 text-xs 
                        ${isFittest ? "bg-green-600" : "bg-blue-800"}`}
										...
										<div className="flex justify-between items-center mt-1 text-xs">
											{viewingHistory ? (
												...
												<span>
													Fitness: {(biomorph.fitness || 0).toFixed(1)}
												</span>
											) : (
						...
						{autoEvolving && biomorphs.length > 12 && (
							<div className="col-span-full text-center text-sm text-gray-400 mt-4">
								Showing 12 of {biomorphs.length} biomorphs for better
								performance.
							</div>
						)}
					</div>
									...
									<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
										...
										{zoomedBiomorph.fitness !== undefined && (
											<div className="bg-gray-900 p-3 rounded">
												...
												<div className="font-bold">
													{zoomedBiomorph.fitness.toFixed(2)}
												</div>
											</div>
										)}
									</div>
				...
				<div className="mt-8 mb-8 bg-gray-800 rounded-xl p-6 border border-gray-700">
					...
					<p className="mb-4">
						This simulation demonstrates Dawkins' Biomorphs - a model of
						evolution through recursive development and selection:
					</p>
					<ul className="list-disc pl-5 space-y-2 mb-4">
						<li>
							<span className="text-blue-400 font-bold">Development:</span> Each
							biomorph's appearance is determined by 9 "genes" that control
							branching patterns.
						</li>
						<li>
							<span className="text-emerald-400 font-bold">Reproduction:</span>{" "}
							When you select a biomorph, it produces 18 offspring (each gene
							can mutate up or down).
						</li>
						<li>
							<span className="text-purple-400 font-bold">Selection:</span> You
							act as the selecting force, choosing which biomorphs survive to
							reproduce.
						</li>
					</ul>
					<p>
						Over generations, you can evolve complex and beautiful structures by
						selecting the biomorphs that appeal to you. This mirrors how
						artificial selection works in breeding plants and animals, and
						serves as a simplified model of natural selection.
					</p>
				</div>

				{/* Footer with attribution */}
				<div className="py-3 text-center text-gray-500 text-xs">
					Biomorphs Evolution Simulator • Built by{" "}
					<a
